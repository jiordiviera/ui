<x-layouts.app>
    <div
        ...
        <aside
            class="fixed top-14 z-30 -ml-2 hidden h-[calc(100vh-3.5rem)] w-full shrink-0 md:sticky md:block overflow-y-auto border-r border-zinc-200 dark:border-zinc-800 pr-4">
            <div class="py-6 pr-6 lg:py-8">
                <h4 class="mb-1 rounded-md px-2 py-1 text-sm font-semibold">Getting Started</h4>
                <div class="grid grid-flow-row auto-rows-max text-sm">
                    <a href="{{ route("docs.show", ['page' => 'installation']) }}"
                        class="group flex w-full items-center rounded-md border border-transparent px-2 py-1 hover:underline {{ (request()->routeIs('docs.show') && request()->route('page') === 'installation') ? 'font-medium text-foreground' : 'text-foreground/80' }}">
                        Installation
                    </a>
                    <a href="{{ route("docs.show", ['page' => 'theming']) }}"
                        class="group flex w-full items-center rounded-md border border-transparent px-2 py-1 hover:underline {{ (request()->routeIs('docs.show') && request()->route('page') === 'theming') ? 'font-medium text-foreground' : 'text-foreground/80' }}">
                        Theming
                    </a>
                </div>
